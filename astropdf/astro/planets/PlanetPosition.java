...
import static astropdf.math.Maths.atan3;
import static astropdf.math.Maths.in2pi;
import static astropdf.math.Maths.radsToDegs;
...
import astropdf.astro.coords.Coord;
import astropdf.astro.coords.EclipticCoords;
import astropdf.astro.coords.LBR;
...
/** 
 The geometric position of a planet at a given time (JDE).
...
public final class PlanetPosition {
  ...
  public LBR lbr(double jde) {
    LBR result = new LBR();
    double τ = AstroUtil.julianMilleniaSinceJ2000(jde);
    result.L = L(τ);
    result.B = B(τ);
    result.R = R(τ);
    return result;
  ...
  public Position position(double jde) {
  ...
  public static void main(String... args) {
    ...
    double jd = GregorianCal.jdForGreenwich(1992, 12, 20.0);
